...
- **Key Decisions Locked**:
  ...
  - Accessibility: VoiceOver reads elements; haptics on likes; high-contrast mode.
...
- **Onboarding & Sign Up (Spotify-Inspired Flow)**:
  - **UX Flow**: Splash ("Your Infinite Music") → "Sign Up Free" button → email/password (or Google/Apple) → ask preferences (multi-select genres/songs/bands, sliders for bass/tempo, toggle explicit) → "Connect accounts for better matches? (Optional)" (Spotify/Apple/YouTube buttons with "Takes 10s—import playlists") → explain benefits ("We'll create infinite music like your favorites") → "Learning your taste..." animation (8s) → home dashboard.
  ...
  - **Figma Screens**: Onboarding_Splash, SignUp_Form, Preferences_Questions (multi-select for songs/bands, sliders for bass/tempo, toggle for explicit), Connect_Buttons, Loading_Animation.
  - **Frontend Code** (Onboarding.tsx – Depth):
    ...
    function PreferencesScreen() {
      ...
      return <View><Slider value={prefs.bass_level} onChange={(val) => setPrefs({...prefs, bass_level: val})} /> <Toggle value={prefs.explicit_lyrics} onChange={(val) => setPrefs({...prefs, explicit_lyrics: val})} /> <Button title="Save" onPress={savePrefs} /></View>;
    }
...
- **Generation Modal (Custom – Extreme Depth)**:
  - **UX**: From player/home → modal with duration slider (15/30/60 min), voice preset picker (12 options), "Generate" → progress ring (<30s) → auto-play + explanation toast. Cool loading UI for V2: Animated waveform "Generating..." with cancel.
...
- **Preferences & Personalization (Depth)**:
  ...
  - **Frontend Code** (Preferences.tsx – Depth):
    ```tsx
    function PreferencesScreen() {
      const [prefs, setPrefs] = useState({ bass_level: 'medium', tempo_range: '120-160', explicit_lyrics: true });
      const save = async () => {
        await api.post('/update_preferences', prefs);
      };
      return <View><Slider value={prefs.bass_level} onChange={(val) => setPrefs({...prefs, bass_level: val})} /> <Toggle value={prefs.explicit_lyrics} onChange={(val) => setPrefs({...prefs, explicit_lyrics: val})} /> <Button title="Save" onPress={save} /></View>;
    }
    ```
  - **Edge Cases**: No prefs? Default to "medium" + infer from first likes.
...
- Legal: 100% original AI; embeds for real (no quota for iframes).
- Scaling: Vercel auto.

#### 6. Business Model, Roadmap, & Metrics
- Freemium: Track daily counts in DB; upsell modals.
...
- Metrics: PostHog for retention/ratings/play time; adds daily counter logic.

This is the full frontend/UI/UX PRD—your team has it all. Send and build!  
